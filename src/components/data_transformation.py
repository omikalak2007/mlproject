...
@dataclass
class DataTransformationConfig:
    preprocessor_ob_file_path = os.path.join(r"C:\mlprojects\artifacts", "preprocessor.pkl")
...
class DataTransformation:
    ...
    def get_data_transformer_object(self):
        """
        Creates and returns a ColumnTransformer object for preprocessing numerical and categorical columns.
        """
        try:
            numerical_columns = ["writing score", "reading score"]
            categorical_columns = [
    ...
    def initiate_data_transformation(self, train_path, test_path):
        """
        Applies preprocessing on the train and test data and saves the preprocessor object.
        """
        try:
            # Load datasets
            train_df = pd.read_csv(train_path)
            ...
            input_feature_train_arr = preprocessing_obj.fit_transform(input_feature_train_df)
            logging.info("Transformation applied on training data.")
            input_feature_test_arr = preprocessing_obj.transform(input_feature_test_df)
            logging.info("Transformation applied on test data.")
            ...
            logging.info(f"Saving preprocessing object at {self.data_transformation_config.preprocessor_ob_file_path}")
            save_object(
                file_path=self.data_transformation_config.preprocessor_ob_file_path,
                obj=preprocessing_obj,
            )
